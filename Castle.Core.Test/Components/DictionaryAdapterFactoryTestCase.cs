...
namespace Castle.Core.Test.Components
{
	...
	public class DictionaryAdapterFactoryTestCase
	{
		...
		public void ReadAdapter_WithDefaultConversions_WorksFine()
		{
			...
			dictionary["Double"] = $"{3.14D}";
			dictionary["Decimal"] = $"{100M}";
		...
		public void UpdateAdapter_WithDefaultConversions_WorksFine()
		{
			...
			Assert.AreEqual($"{3.14D}", dictionary["Double"]);
			Assert.AreEqual($"{100M}", dictionary["Decimal"]);
		...
		public void ReadAdapter_WithDefaultNullableConversions_WorksFine()
		{
			...
			dictionary["NullDouble"] = $"{3.14D}";
			dictionary["NullDecimal"] = $"{100M}";
		    ...
		    if (conversions.NullDateTime != null) Assert.AreEqual(now.Value.Date, conversions.NullDateTime.Value.Date);
		...
		public void UpdateAdapter_WithDefaultNullableConversions_WorksFine()
		{
			...
			Assert.AreEqual($"{3.14D}", dictionary["NullDouble"]);
			Assert.AreEqual($"{100M}", dictionary["NullDecimal"]);
#if SILVERLIGHT // SL impl limitation
